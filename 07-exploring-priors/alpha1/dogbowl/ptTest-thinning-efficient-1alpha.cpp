...
SEXP ptTest(NumericMatrix train, NumericMatrix test,
  ...
  if((csamp == 1.0) & ((cpar1 < 0) | (cpar2 <0))){
    cpar1 = 1.0;
    cpar2 = 1.0/chooseC(rij    , z    , J, ldet);
  ...
  for(int i=1; i<=mcmc; i++){
    ...
    if(sigmarand){
      ...
      //update sigma
      sigmaPROPARM = riwish(df,(df-dim-1)*sigmaARM);
      ...
      // Updating
      numerator   = propLL +  as_scalar(diwish(sigmaPROPARM, ntrain,(ntrain-dim-1)*datacovmatARM,true)) + as_scalar(diwish(sigmaARM,      df,(df-dim-1)*sigmaPROPARM,    true));
      denominator = currLL +  as_scalar(diwish(sigmaARM, ntrain,(ntrain-dim-1)*datacovmatARM,true)) + as_scalar(diwish(sigmaPROPARM, df,(df-dim-1)*sigmaARM,        true));
