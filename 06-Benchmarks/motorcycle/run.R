...
train = curr.train %>% dplyr::select(accel, times)
test = curr.test %>% dplyr::select(accel, times)
###########################################################################
###########################################################################
# Fix Models
###########################################################################
...
## lm
ols.mod <- lm(frmla, data = train)
ols.pred <- predict(ols.mod, newdata = test)

## quantile
quant.mod <- rq(frmla, data=train)
quant.pred <- predict(quant.mod, newdata=testx)

# svm linear
svm.linmod <- svm(frmla, data = train, kernel = "linear")
svm.linpred <- predict(svm.linmod, newdata = testx)

# svm polu
svm.polymod <- svm(frmla, data = train, kernel = "polynomial")
svm.polypred <- predict(svm.polymod, newdata = testx)

# svm radial
svm.radmod <- svm(frmla, data = train, kernel = "radial")
svm.radpred <- predict(svm.radmod, newdata = testx)

# svm sigmoid
svm.sigmod <- svm(frmla, data = train, kernel = "sigmoid")
svm.sigpred <- predict(svm.sigmod, newdata = testx)
...
# GBoost
gbm.mod<-gbm(frmla, data = train) # 100 trees
gbm.pred<-predict(gbm.mod, newdata = testx)
...
#BART
bm<-bartMachine(trainx,trainy,num_trees = 100)
bart.pred<-predict(bm, testx)

#Cart
cart.model<-rpart(frmla, data=train)
cart.pred<-predict(cart.model, testx)
