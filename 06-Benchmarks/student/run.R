...
frmla <- "y ~ x1"
gamfrmla <- "y ~ s(x1)"
gamsing <- c(3, 7:8, 13:15, 24:29) # resolve gam error:   A term has fewer unique covariate combinations than specified maximum degrees of freedom
if(length(cnames)>2){
  for(i in 2:(length(cnames)-1)){
    frmla <- paste(frmla, " + x" , i, sep="")
    if(i %in% gamsing){
      gamfrmla <- paste(gamfrmla, " + x" , i, "", sep="") 
    }else{
