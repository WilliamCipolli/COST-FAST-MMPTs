...
#Compare Sampling
################################################################
df.fast <- tibble(Iterate = (1:10000) * 100,
...
mu1 <- ggplot()+
  ...
  scale_x_continuous(breaks=seq(0,1000000, length.out=5),
                     ...
                     sec.axis = sec_axis(~., name = "FAST Iterate",
                                         breaks = seq(0,1000000, length.out=5),
                                         labels = seq(0,1000000, length.out=5)/100))
...
min.cp <- floor(min(0, min(df.mcmc$cp), min(df.fast$cp)))
max.cp <- ceiling(max(1, max(df.mcmc$cp), max(df.mcmc$cp)))
...
cp2 <- ggplot() +
  ...
  #xlim(min.cp, max.cp)+
  ylab("Density")+
  scale_x_continuous(breaks = c(0, 0.5*10^306, 1*10^306, 1.5*10^306),
                     labels = c("0",'5.0e305', "1.0e306", "1.5e306"))+
  scale_y_continuous(breaks = c(0, 2.5*10^(-229), 5.0*10^(-229), 7.5*10^(-229), 10*10^(-229)),
                     labels = c("0", "2.5e-229", "5.0e-229", "7.5e-229", "10.5e-229"))
...
g +
  ...
  ylab("Density Difference (FAST - MCMC)")
dev.off()
...
p1 <- ggplot(data=ggdat1) +
  ...
  geom_hline(yintercept=0)+
  theme_bw()+
  xlab("Density Difference (FAST - MCMC)")+
  ylab("Density")
...
p2 <- ggplot(data=ggdat2) +
  ...
  theme_bw()+
  scale_x_continuous("Density Estimate (FAST)",
  ...
  scale_y_continuous("Density Estimate (MCMC)",
                     limits = c(mindens, maxdens),
                     breaks = seq(mindens, maxdens, length.out=5),
                     labels = format(round(seq(mindens, maxdens, length.out=5),3), nsmall=3))
